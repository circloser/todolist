  ...
  return (
    <div className="space-y-5">
      ...
      <Section icon="📋" title="업무 추가와 진행 체크">
        ...
        <p>
          <strong>대분류(그룹)</strong>: 업무는 <strong>대분류</strong>로 묶여
          목록에서 접이식 그룹 섹션으로 표시됩니다(그룹별 완료 수·진행률 표시).
          추가 폼이나 업무 상세의 &lsquo;대분류&rsquo;에 이름을 입력하면 되고
          (비우면 <strong>미분류</strong>), 그룹 헤더의 <Key>＋ 업무</Key>를
          누르면 그 그룹으로 바로 새 업무를 추가합니다. 그룹 헤더 왼쪽{" "}
          <Key>⋮⋮</Key> 핸들을 드래그하면 <strong>대분류 순서</strong>를 바꿔
          팀 전체에 저장할 수 있습니다. 대분류는{" "}
          <strong>유형(단계 프리셋)</strong>과 별개입니다 — 유형은 진행 절차를,
          대분류는 업무 묶음을 정합니다.
        </p>
        <p>
        ...
        <p>
          <strong>세부 체크리스트</strong>: 단계보다 작은 할 일을 추가합니다.
          각 단계 행의 <Key>☑</Key> 배지를 누르면 그{" "}
          <strong>단계별 세부 항목</strong>을 추가·체크·삭제할 수 있고
          (배지에 완료/전체 수가 표시), 단계와 무관한 항목은 아래{" "}
          <strong>공통 세부 체크리스트</strong>에 둡니다.
        </p>
        <p>
