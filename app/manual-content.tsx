...
export default function ManualContent() {
  return (
    <div className="space-y-5">
      ...
      <Section icon="🏢" title="연도/팀 보드와 암호">
        <p>
          첫 화면에서 <Key>연도</Key>와 <Key>팀</Key>을 선택하고 암호를 입력해
          입장합니다. 암호를 통과해야 업무 목록·지도·보고서를 볼 수 있습니다.
        </p>
        <p>
          새 연도/팀은 첫 화면의 <Key>새 연도/팀 생성</Key>에서 추가합니다.
          암호는 4자 이상이어야 하며, 입력한 암호는 현재 브라우저 세션에만
          기억됩니다.
        </p>
        <p>
          생성할 때 <Key>전년도 같은 팀 데이터 가져오기</Key>를 선택하면 전년도
          업무와 단계, 세부 체크리스트, 담당자 색상, 그룹 설정을 새 보드로
          복사합니다. 전년도 보드가 암호로 잠겨 있으면 전년도 암호가 필요합니다.
        </p>
        <p>
          현재 보드의 연도, 팀명, 새 암호는 화면 하단 <strong>보드 설정</strong>의{" "}
          <Key>현재 연도/팀</Key> 영역에서 변경합니다. 암호 칸을 비워 두고 저장하면
          이름만 변경됩니다.
        </p>
      </Section>
      ...
      <Section icon="📂" title="업무 위계와 목록 사용">
        ...
        <p>
          ...
          헤더의 <Key>✎</Key>는 이름 수정, <Key>×</Key>는 대분류 삭제입니다.
          업무 카드를 다른 그룹으로 끌면 해당 대분류로 이동하고, 그룹을 다른
          그룹 위에 놓으면 합치기 또는 순서 이동을 선택합니다.
        </p>
      </Section>
      ...
      <Section icon="📋" title="업무 추가·수정·삭제">
        ...
        <p>
          업무 행을 클릭하면 상세가 펼쳐집니다. 상세에서는 업무명, 담당자 색상, 업무
      ...
      <Section icon="🗺" title="지도">
        <p>
          ...
          핀은 마우스로 끌어 원하는 위치로 옮길 수 있고, 좌표가 바로 저장됩니다.
        </p>
        <p>
        ...
        <p>
          <strong>지도 설정</strong>에서 Leaflet/OSM, Google, VWorld와 일반·위성·
          하이브리드 지도를 선택합니다. VWorld와 Google은 API 키를 입력하면
          현재 PC 브라우저에 저장됩니다. <strong>지명 검색</strong>으로 장소를
          찾아 지도 중심을 이동할 수 있습니다.
        </p>
        <p>
      ...
      <Section icon="📄" title="월간 보고서">
        <p>
          ...
          컬럼 헤더를 누르면 대분류·업무명·예산·담당·진행률·마감일·상태 기준으로
          정렬됩니다.
        </p>
        <p>
      ...
      <Section icon="💡" title="운영 팁">
        ...
        <p>
          · 업무 데이터는 선택한 연도/팀 보드별로 나뉘어 팀 공용 서버에
          저장됩니다. 사용자명과 대시보드 위젯 표시 설정은 각 브라우저에
          저장됩니다.
        </p>
        <p>
